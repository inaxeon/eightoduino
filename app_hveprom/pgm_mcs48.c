...
// If having difficulties - try increasing the below 4 delays
...
void pgm_mcs48_write_chunk(void)
{
    ...
    for (i = 0; i < thisChunk; i++)
    {
        ...
        if (slow)
            pgm_mcs48_delay_pre_address_latch();
        pgm_mcs48_delay_4tcy(); // Multi-millisecond delay observed to be required here on CMOS parts
        pgm_mcs48_reset_disable();
        if (slow)
            pgm_mcs48_delay_post_address_latch();
        pgm_mcs48_delay_4tcy(); // Multi-millisecond delay observed to be required here on CMOS parts
